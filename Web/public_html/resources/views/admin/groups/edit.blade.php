...
<div class="p-8 bg-[#0a0a0a] min-h-screen">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-10">
            <a href="{{ route('user.groups') }}" class="p-2 bg-[#1a1a1a] rounded-lg text-[#4a4a4a] hover:text-white transition-all">
            ...
            <form action="{{ route('user.groups.update', $group->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="flex flex-col items-center gap-4">
                    <div class="w-32 h-32 rounded-3xl bg-[#0a0a0a] border-2 border-dashed border-[#2a2a2a] overflow-hidden flex items-center justify-center relative group">
                        @if($group->icon_url)
                            <img src="{{ str_contains($group->icon_url, 'http') ? $group->icon_url : asset('storage/' . $group->icon_url) }}" class="w-full h-full object-cover opacity-60">
                        @else
                    ...
                    <p class="text-[10px] font-bold text-[#4a4a4a] uppercase tracking-widest text-center">Recommended: 512x512 PNG/JPG</p>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="target_km" step="0.1" min="0" value="{{ $group->target_km ?? '' }}"
                           placeholder="e.g. 100"
                    ...
                    <p class="text-[#6b6b4b] text-xs mt-2">Set or update the collective monthly goal for your group members</p>
                </div>
