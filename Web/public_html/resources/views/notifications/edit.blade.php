...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-8">
        <h2 class="text-2xl font-bold text-white mb-6">Edit Notification</h2>

        <form action="{{ route('notifications.update', $notification->id) }}" method="POST" class="space-y-6">
            @csrf @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <textarea name="message" rows="3" required
                          class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white rounded-lg px-4 py-3 focus:outline-none focus:border-[#6b6b4b]">{{ $notification->message }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="datetime-local" name="scheduled_at" value="{{ $notification->scheduled_at->format('Y-m-d\TH:i') }}"
                ...
                <div class="space-y-2">
                    <label class="text-sm font-medium text-[#b0b0a0]">Recipient Count</label>
                    <input type="number" name="recipients_count" value="{{ $notification->recipients_count }}"
                           class="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white rounded-lg px-4 py-3 focus:outline-none focus:border-[#6b6b4b]">
                </div>
            </div>
